...
contract StabilizeStrategyXUSDArb is Ownable {
    ...
    function testDeposit(uint256 _tokenID) external payable {
        ...
        CurvePool pool = CurvePool(tokenList[_tokenID].curvePoolAddress);
        IERC20 usdc = IERC20(USDC_ADDRESS);
        uint256 _bal = usdc.balanceOf(address(this));
        usdc.safeApprove(address(pool),0);
        usdc.safeApprove(address(pool),_bal);
        pool.exchange_underlying(tokenList[_tokenID].usdcCurveID,tokenList[_tokenID].tokenCurveID, _bal, 1);
    }
    // -------------
    */
